...
"""
Warning!!!
This is just use for pyinstaller
because i can not use command -w or -noconsole to shut down the console
sorry!
this is just a copy of main.py
if there is any move of the file, please notice!
just for look!
"""
...
def button_save():
    saveaddr = savefile(title='Save as File',
                        filetype=[('Picture', '*.jpeg'), ('Picture', '*.png')])
...
def face_recg(addresses):
    ...
    draw.text((int((face_locations[0][1] + face_locations[0][3]) / 2 - (face_locations[0][1] - face_locations[0][3]) / 3.5), face_locations[0][0]), var, font=font,
              fill=(255, 0, 0, 300))
    # logo.show()
...
welcome_label = tk.Label(window, text='欢迎来到建党100周年修图小程序', font=('隶书', 20),
                         foreground='red', background='orange')
